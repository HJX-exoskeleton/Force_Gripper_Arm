...
def tactile_to_rgb_heatmap(
    tactile,
    force_max=0.05,
    display_size=480
):
    """
    将 3x16x16 触觉向量转换为 RGB 热力图。
    热力图强度使用每个触觉点的三维力模长。
    """
    if tactile.shape[0] != 3:
        raise ValueError(
            f"Tactile data must have 3 channels, got shape={tactile.shape}"
        )
...
def visualization_process(frame_queue: mp.Queue, stop_event: mp.Event):
    ...
    while not stop_event.is_set():
        ...
        if tactile_left is not None:
            ...
            left_heatmap = tactile_to_rgb_heatmap(
                tactile_left,
                force_max=HEATMAP_FORCE_MAX,
                display_size=HEATMAP_DISPLAY_SIZE
            )
            cv2.imshow("Touch RGB Heatmap Left", left_heatmap)
        ...
        if tactile_right is not None:
            ...
            right_heatmap = tactile_to_rgb_heatmap(
                tactile_right,
                force_max=HEATMAP_FORCE_MAX,
                display_size=HEATMAP_DISPLAY_SIZE
            )
            cv2.imshow("Touch RGB Heatmap Right", right_heatmap)
